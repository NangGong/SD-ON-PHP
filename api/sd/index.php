...
function aijianhuang($url) {
    $mgc = "https://api.oioweb.cn/api/ai/jianhuang?url=".$url;
    ...
    // 使用创建的流上下文，获取URL内容
    $response = file_get_contents($mgc, false, $context);
    $response = json_decode($response, true);

    if($response['code'] == 200) {
        if($response['rating_index'] == 3) {
            echo json_encode(array('success' => false, 'message' => '检测到图片为不良图片，请重试'));
            exit;
        } else {
            echo json_encode(array('success' => true, 'url' => $response['url_classified']));
            exit;
        }

    } else {
        echo json_encode(array('success' => false, 'message' => '请求失败，请重试'));
        exit;
    }
...
if($PaintingKey == "") {
    ...
    //echo json_encode(array('success' => false, 'message' => '暂时关闭接口'));
    // return;
